...
if __name__ == '__main__':
    ...
    DATASET = 'SPIRAL'
    experiment_name = 'SPIRAL variational multiple shooting 20 vars, 1024 samples, 2T, 1e-2 lr Adam 0.9999 scheduler 5 layers, linear rhs all losses (+ 1e-7 wd) 1e-5 incr, Tanh decoder, 20 latent dim'
    ...
    training_params = {'lambda1': 1e-9, 'lambda2': 1e-9, 'lambda3': 1e-9, 'l2_lambda': 0,
    ...
    # shooting = LatentSingleShooting(signal_dim=2, latent_dim=100)
    # shooting = SingleShooting(len(trajectory.visible_dims))
    # shooting = LatentMultipleShooting(signal_dim=2, latent_dim=100, n_shooting_vars=20)
    # shooting = LatentMultipleInterShooting(signal_dim=2, latent_dim=15, n_shooting_vars=41)
    shooting = VariationalLatentMultipleShooting(signal_dim=2, latent_dim=20, n_shooting_vars=20, n_samples=1024)
